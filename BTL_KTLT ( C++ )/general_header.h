...
void  inPhieuDiemCaNhan(const Student& student, const Diem& diem, int STT) {
    cout << endl;
    cout << "\t\t\t\t\tPHIEU BAO DIEM\n";
    ...
    // Header bảng điểm
    cout << "------------------------------------------------------------------------------" << endl;
    cout << "|STT |" << left << setw(10) << "Mon hoc" << "|";
    for (int j = 1; j <= 4; j++) {
        cout << "Diem " << j << setw(5) << "|"; // Sử dụng setw để căn chỉnh
    }
    ...
    for (int i = 0; i < 12; i++) {
        // Lấy điểm trung bình môn học từ mảng ketQuaHocSinh
        float diemTBMon = ketQuaHocSinh[i].getDiemTrungBinhMonHoc(i);
        tongDiemTB += diemTBMon;

        // In điểm từng môn học
        cout << "|" << setw(4) << i + 1 << " |"
             << setw(10) << subjectNameToString(student.getSubject(i).subject_name) << "|";
        for (int j = 0; j < 4; j++) {
            cout << setw(5) << fixed << setprecision(1) << student.getSubject(i).score.test_mini[j] << "|";
        }
        for (int j = 0; j < 2; j++) {
            cout << setw(5) << fixed << setprecision(1) << student.getSubject(i).score.test_45mins[j] << "|";
        }
        cout << setw(5) << fixed << setprecision(1) << student.getSubject(i).score.mid_term_score << "|"
             ...
             << setw(5) << fixed << setprecision(1) << diemTBMon << " |" << endl; 
    }
